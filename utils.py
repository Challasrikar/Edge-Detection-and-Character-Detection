...
def flip2d(img, axis=None):
    ...
    if(axis==0):
        ...
        for i in range(center):
            flipped_img[i] = img[(len(img) - 1) - i]
            flipped_img[(len(img) - 1) - i] = img[i]
        flipped2_img = copy.deepcopy(flipped_img)
        for i, row in enumerate(flipped_img):
            for j in range(center):
                flipped2_img[i][j] = flipped_img[i][(len(flipped_img[0]) - 1) - j]
                flipped2_img[i][(len(flipped_img[0]) - 1) - j] = flipped_img[i][j]
